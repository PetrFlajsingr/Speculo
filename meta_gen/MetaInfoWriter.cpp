...
namespace pf::meta_gen {
    ...
    void MetaInfoWriter::writeEnumInfo(const EnumTypeInfo &enumInfo) {
        ...
        const auto const_type_id = idToString(idGenerator->generateTypeId("const " + enumInfo.fullName));
        const auto lref_type_id = idToString(idGenerator->generateTypeId(enumInfo.fullName + "&"));
        const auto const_lref_type_id = idToString(idGenerator->generateTypeId("const " + enumInfo.fullName + "&"));
        const auto rref_type_id = idToString(idGenerator->generateTypeId(enumInfo.fullName + "&&"));
        const auto ptr_type_id = idToString(idGenerator->generateTypeId(enumInfo.fullName + "*"));
        const auto const_ptr_type_id = idToString(idGenerator->generateTypeId("const" + enumInfo.fullName + "*"));
